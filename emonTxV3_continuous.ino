...
// ----------------- RF setup  ---------------------
#define RF_freq RF12_433MHZ   // Use the RF_freq to match the module you have.
...
const float voltageCal         = (225.2/7.85)*12.87;
const float normalisation_Vrms = ((3.3/1024)*voltageCal*1.11)/(DATALOG_PERIOD_IN_SECONDS * CYCLES_PER_SECOND);
...
const float powerCal_CT1 = (voltageCal*(3.3/1024))*(90.9*(3.3/1024)); // <---- powerCal value
const float powerCal_CT2 = (voltageCal*(3.3/1024))*(90.9*(3.3/1024)); // <---- powerCal value
const float powerCal_CT3 = (voltageCal*(3.3/1024))*(90.9*(3.3/1024)); // <---- powerCal value
const float powerCal_CT4 = (voltageCal*(3.3/1024))*(16.6*(3.3/1024)); // <---- powerCal value (2000 / 120R burden resistor)
...
{  
  ...
  Serial.print ( "voltageCal   =  "); Serial.println (voltageCal);
  Serial.println("----------------------------------");    
...
void allGeneralProcessing()
{
  ...
  {                           
    ...
    { 
      // Add voltage samples for every positive cycle
      sumVrms += sampleVminusDC_long; 
      if (polarityOfLastSampleV != POSITIVE)
      {
        ...
        // Average RMS voltage during one measurement cycle
        voltageAccumulator      += ((int)(sumVrms >> 7) / (byte)(samplesDuringThisCycle));
        ...
        {         
          ...
          tx_data.Vrms      = voltageAccumulator * normalisation_Vrms;
          boolean OKtoSend;
